          ...
          <div className="p-5 space-y-4 overflow-y-auto flex-1">
            ...
            <div>
              <label className="block text-xs font-medium text-[#999999] mb-2">
                Links do projeto
                <span className="ml-1.5 text-[#666666] font-normal">URLs ou caminhos de pastas</span>
              </label>
                  ...
                  {links.map((link) => (
                    <div key={link.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1b1b1b] border border-[#3b3b3b] group">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-[#d4d4d4] font-medium truncate">{link.label}</p>
                        <p className="text-[11px] text-[#666666] truncate">{link.url}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => handleRemoveLink(link.id)}
                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded text-[#999999] hover:text-[#e04040]"
                      >
            ...
            <div>
              <label className="block text-xs font-medium text-[#999999] mb-2">
                Caminhos de código
                <span className="ml-1.5 text-[#666666] font-normal">pastas no PC onde a IA trabalha</span>
              </label>
                            ...
                            >
                              <span
                                className={`shrink-0 flex items-center justify-center w-4 h-4 rounded border-2 ${
                                  active ? 'border-[#7c3aed] bg-[#7c3aed]' : 'border-[#666666]'
                                }`}
                              >
                                {active && (
                              ...
                              <span className="flex-1 min-w-0">
                                {cp.label && (
                                  <span className="block text-xs text-[#d4d4d4] font-medium truncate">{cp.label}</span>
                                )}
