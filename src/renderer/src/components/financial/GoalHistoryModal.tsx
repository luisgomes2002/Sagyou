...
export function GoalHistoryModal({ open, goals, transactions, accBalance, currency, onRevert, onClose }: GoalHistoryModalProps) {
  ...
  const statusCfg: Record<string, { label: string; bg: string; text: string }> = {
    concluded: { label: 'Concluído', bg: 'bg-[#20b858]/15', text: 'text-[#46d478]' },
    achieved: { label: 'Alcançado', bg: 'bg-[#20b858]/15', text: 'text-[#20b858]' },
      ...
      <div className="relative z-10 w-[580px] max-h-[75vh] rounded-xl border border-[#3b3b3b] bg-[#232323] shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#3b3b3b]">
          {withStatus.map(({ goal, status, progress }) => {
            ...
            return (
              <div key={goal.id} className="flex items-center gap-4 px-5 py-3 hover:bg-[#2a2a2a] transition-colors">
                <div className="relative shrink-0 w-9 h-9">
                  <svg width="36" height="36" viewBox="0 0 36 36">
                    ...
                    <circle
                      cx="18" cy="18" r="13"
                      fill="none"
                      stroke={status === 'overdue' ? '#e04040' : status === 'urgent' ? '#f08a34' : '#20b858'}
                      strokeWidth="3"
                      strokeLinecap="round"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[#999999]">
                    ...
                    {goal.completedAt && (
                      <span className="text-[#46d478]"> · Concluído em {formatDateBR(goal.completedAt)}</span>
                    )}
                  </p>
                  {goal.completionNote && (
                    <p className="text-[10px] text-[#999999]/60 mt-0.5 truncate">{goal.completionNote}</p>
                  )}
                </div>

                <div className="text-right shrink-0 flex flex-col items-end gap-1.5">
                  <p className="text-xs font-bold text-[#d4d4d4] tabular-nums">{formatCurrency(goal.targetAmount, currency)}</p>
                  <p className="text-[10px] text-[#999999]">{MONTH_NAMES[goal.targetMonth - 1].slice(0, 3)} {goal.targetYear}</p>
                  {status === 'concluded' && (
                    <button
