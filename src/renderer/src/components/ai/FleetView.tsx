...
function formatProgress(step: number, maxSteps: number): string {
  return maxSteps === 0 ? String(step) : `${step}/${maxSteps}`
}
...
function useCodeAgentRuns(): {
  ...
  useEffect(() => {
    ...
    const offExit = window.electronAPI.ai.codeAgent.onExit(
      (_payload: { runId: string; code: number }) => {
        setRuns((prev) => prev.map((r) => (r.id === _payload.runId ? { ...r, approvals: [] } : r)))
        sync()
      }
    )
          ...
          prev.map((r) =>
            r.id === payload.runId
              ? {
                  ...
                  approvals: (r.approvals ?? []).filter(
                    (a) => !payload.resolvedApprovalIds?.includes(a.id)
                  )
                }
              : r
          )
    ...
    const offApproveRequest = window.electronAPI.ai.codeAgent.onApproveRequest((req) => {
      setRuns((prev) =>
        prev.map((r) =>
          r.id === req.runId
            ? {
  ...
  const setAuto = useCallback(
    (runId: string, enabled: boolean): void => {
      window.electronAPI.ai.codeAgent.setAuto(runId, enabled).then(() => {
        sync()
      })
    },
    [sync]
...
export function FleetView({
  ...
  const [runCache, setRunCache] = useState<Map<string, { log: string; diff: CodeAgentDiff }>>(
    new Map()
  )
  // ── Active agent chat ────────────────────────────────────────────────────────
  const [chatSession, setChatSession] = useState<ChatSession | null>(null)
  const [chatMessages, setChatMessages] = useState<
    { role: 'user' | 'agent'; text: string; diff?: CodeAgentDiff; log?: string; runId?: string }[]
  >([])
  ...
  useEffect(() => {
    window.electronAPI.ai.codeAgent
      .runs()
      .then((runs) =>
        setArchivedRuns(runs.sort((a: AgentRunMeta, b: AgentRunMeta) => b.startedAt - a.startedAt))
      )
  }, [codeRuns.length])
  ...
  useEffect(() => {
    const offArchived = window.electronAPI.ai.codeAgent.onArchived(async (payload) => {
      ...
      try {
        ...
        if (snap) {
          const diff = (snap.diff as CodeAgentDiff) ?? {
            patch: '',
            files: [],
            truncated: false,
            omittedNewFiles: []
            ...
            return [
              ...prev,
              {
                ...
                text:
                  snap.exitCode === 0
                    ? 'Agente concluiu.'
                    : `Agente encerrou com código ${snap.exitCode}.`,
                diff,
                log,
                runId: payload.id
              }
  ...
  const toggleRun = useCallback(
    async (id: string): Promise<void> => {
      ...
      setRunCache((prev) => {
        ...
        next.set(id, {
          log: snap.log ?? '',
          diff: (snap.diff as CodeAgentDiff) ?? {
            patch: '',
            files: [],
            truncated: false,
            omittedNewFiles: []
   ...
   */
  const loadChatSessionHistory = useCallback(async (convId: string): Promise<void> => {
    try {
  ...
  const codeAgentProject = (dir: string): string => {
    const match = projects.find((p) => p.codePaths?.some((cp) => dir.includes(cp.path)))
          ...
          <>
            ...
            <span className="text-xs text-[#999999]">
              ...
              {chatAgents.length > 0 && codeAgents.length > 0 && (
                <>
                  {' '}
                  · {chatAgents.length} chat{chatAgents.length !== 1 ? 's' : ''} ·{' '}
                  {codeAgents.length} código
                </>
              )}
            </span>
          </>
                ...
                >
                  ...
                  {m.log && (
                    <details className="group mt-2">
                      ...
                      <div className="mt-1">
                        <AgentTerminal log={m.log} running={false} />
                      </div>
                    </details>
                  )}
                </div>
                ...
                onKeyDown={async (e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    ...
                    try {
                      ...
                      if (result.runId) {
                        setChatSession((prev) =>
                          prev ? { ...prev, runId: result.runId ?? null } : prev
                        )
                      }
                    } finally {
            ...
            {chatAgents.length === 0 && codeAgents.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-[#999999] max-w-sm">
                  Comece uma conversa na aba <b>IA</b>. Vários agentes podem trabalhar ao mesmo
                  tempo, inclusive no mesmo projeto — todos aparecem aqui.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {chatAgents.map((id) => {
                  const projectName =
                    projects.find((p) => p.id === runProjects[id])?.name ?? 'Global'
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 text-[11px] tabular-nums">
                        <span
                          ...
                          title="tokens de entrada (prompt) desta execução"
                        >
                          ↑ {formatTokens(tokens.promptTokens)} entrada
                        </span>
                        <span
                          className="text-[#999999]"
                          title="tokens de saída (resposta) desta execução"
                        >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        {hasApproval ? (
                          ...
                          <span
                            className="w-2.5 h-2.5 shrink-0 rounded-full bg-[#f0b820]"
                            title={run.hint!.detail}
                          />
                        ) : run.progress ? (
                          <span
                            className="w-2.5 h-2.5 shrink-0 rounded-full border-[1.5px] border-[#a080f0] border-t-transparent animate-spin"
                            title={`Passo ${formatProgress(run.progress.step, run.progress.maxSteps)}`}
                          />
                        ) : (
                          <span
                            className="w-2.5 h-2.5 shrink-0 rounded-full border-[1.5px] border-[#a080f0] border-t-transparent animate-spin"
                      ...
                      {run.progress && (
                        <div className="flex items-center gap-3 text-[11px] tabular-nums">
                          <span className="text-[#999999]">
                            ↑ {run.progress.promptTokens.toLocaleString()} entrada
                          </span>
                          <span className="text-[#999999]">
                            ↓ {run.progress.completionTokens.toLocaleString()} saída
                          </span>
                        </div>
                      )}
                      ...
                      {hasApproval && (
                        <div className="space-y-2 rounded-lg border border-[#f0b820]/25 bg-[#f0b820]/5 p-2">
                          {run.approvals.map((approval) => (
                            ...
                            >
                              <span
                                className="min-w-0 truncate text-[#d4d4d4]"
                                title={approval.resumo}
                              >
                                {approval.resumo}
                              </span>
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        <ModeToggleButton
                          active={run.autoApprove}
                          label="Auto"
                          activeLabel="Auto: ON"
                          onClick={() => setAuto(run.id, !run.autoApprove)}
                          title={
                            run.autoApprove
                              ? 'Modo autônomo LIGADO — clique para voltar a pedir aprovação.'
                              : 'Modo autônomo DESLIGADO — clique para aprovar ações automaticamente.'
                          }
                        />
                        <button
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-[#999999] mt-1">
                            {run.agent} · {run.fileCount}{' '}
                            {run.fileCount === 1 ? 'arquivo' : 'arquivos'}
                            {run.tokens &&
                              ` · ${((run.tokens.promptTokens + run.tokens.completionTokens + (run.tokens.reasoningTokens ?? 0)) / 1000).toFixed(0)}k tok`}
